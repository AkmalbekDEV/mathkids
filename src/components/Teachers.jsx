    ...
    <section id='teachers' className='max-w-[1250px] mx-auto'>
      <div className='py-20 grid gap-5'>
        ...
        <div className='flex items-center justify-center flex-wrap gap-10'>
          {data.map((item) => (
            <div key={item.id} className='grid content-between w-[280px] h-[450px] border-2 rounded-xl p-5 max-lg:mx-5'>
              <img src={item.imageURL} alt={item.name} className='w-[250px] h-[250px] rounded-xl' />
              <h1 className='text-center font-medium text-xl'>{item.name}</h1>
              <h1 className='text-center text-lg'>{item.job}</h1>
